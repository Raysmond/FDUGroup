...
<div class="panel panel-default">
    ...
    <div class="panel-body" id="help-list">
        ...
        <?=RHtml::image("files/images/site/01.png","",array('style'=>"width: 100%;"))?>
        ...
        <?=RHtml::image("files/images/site/08.png","",array('style'=>"width: 100%;"))?>
        你可以在该话题下与其他人讨论、交流以及点赞(绿色标记部分)：

        <?=RHtml::image("files/images/site/09.png","",array('style'=>"width: 100%;"))?>


        你也可以创建新的话题：
            <?=RHtml::image("files/images/site/10.png","",array('style'=>"width: 100%;"))?>
        如下图所示，赶快发表你的第一个话题吧~
        <?=RHtml::image("files/images/site/11.png","",array('style'=>"width: 100%;"))?>
        </div>
        ...
        本条目将指引你进行朋友管理，包括查找好友，新增好友，查看好友信息等操作。在个人主页下，点击“find friend”按钮（黄色标记部分），即可看到系统之中所有的用户，您可以通过直接输入用户名进行搜索。
            ...
            <?=RHtml::image("files/images/site/12.png","",array('style'=>"width: 100%;"))?>

        查找到您感兴趣的用户后，点击该用户头像，在该用户公开信息中，点击“add friend”即可申请好友：
            <?=RHtml::image("files/images/site/13.png","",array('style'=>"width: 100%;"))?>

        你也可以在这里查看该用户的公开信息。对于已成为好友的用户，您可以向对方发送message或解除好友关系：
            <?=RHtml::image("files/images/site/14.png","",array('style'=>"width: 100%;"))?>
        ...
        该条目将指导你对收到的信息进行管理，以及如何发送信息。
            ...
            <?=RHtml::image("files/images/site/15.png","",array('style'=>"width: 100%;"))?>
        如图所示，你可以在个人主页中对信息进行管理，在主页messages右侧，有一个标签，提示当前有多少条未读消息（黄色标记部分），你可以对收到的信息进行管理，如查看、回复、标记为已读、删除到垃圾箱中等（绿色标记部分）。你也可以对其他人发送消息（红色标记部分），界面如下图所示：
            <?=RHtml::image("files/images/site/16.png","",array('style'=>"width: 100%;"))?>
        你可以向任何已添加为好友的用户发送消息。
        </div>
        ...
        该条目将指引你对个人信息进行修改。如图，在个人主页下点击my profile即可查看个人信息，如下图所示：
            <?=RHtml::image("files/images/site/17.png","",array('style'=>"width: 100%;"))?>
        ...
        注意！只有VIP用户才有资格申请发布广告。关于如何成为VIP用户，详见profile指引。
            <?=RHtml::image("files/images/site/18.png","",array('style'=>"width: 100%;"))?>
        如上图所示，单击个人主页的“advertisement”条目，界面如上图所示。在这里，您可以发布诸如征友、销售、教育等广告信息，通过publish按钮提交，待管理员审核通过后，即可在屏幕右下方看到您的广告，如下图所示：
            <?=RHtml::image("files/images/site/19.png","",array('style'=>"width: 100%;"))?>
