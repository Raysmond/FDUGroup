...
<div class="panel panel-default">
    ...
    <div class="panel-body" id="help-list">
        ...
        <?=RHtml::showImage("files/images/site/01.png","",array('style'=>"width: 100%;"))?>
        ...
        找到感兴趣的小组后，点击该小组下侧导航栏的“+”按钮，向该小组管理者发出申请加入小组的请求，待对方审核通过后，即可加入该小组，进行讨论和发表话题。
            <?=RHtml::showImage("files/images/site/02.png","",array('style'=>"width: 100%;"))?>
        </div>

        <div class="row" id="groups-management">
            <h2>Groups management</h2>
        本条目将指导您如何对已加入的小组进行管理。单击个人帐号下的home page，进入如下界面后，点击右侧导航栏的“my groups”条目，即可进入group管理界面。
            <?=RHtml::showImage("files/images/site/03.png","",array('style'=>"width: 100%;"))?>
        每个小组的下方提供了三个按钮（黑色标记部分），其中左侧人物头像表示该小组当前人数，心号表示有多少人认为该小组很赞！您也可以对该小组进行点赞。点击右侧“X”将退出该小组。
        </div>

        <div class="row" id="create-groups">
        <h2>Create Groups</h2>
            <?=RHtml::showImage("files/images/site/04.png","",array('style'=>"width: 100%;"))?>
        本条目将指导您创建一个小组。在个人主页下，点击如图红色标记部分的“new group”按钮即可创建新的小组，创建界面如下图所示：
        <?=RHtml::showImage("files/images/site/05.png","",array('style'=>"width: 100%;"))?>

        按照上述步骤进行操作，即可获得您在FDUGroup创建的第一个小组了！对于自己创建的小组，您还可以对小组进行后期的编辑，并且邀请好友加入小组。如下图所示：
        <?=RHtml::showImage("files/images/site/06.png","",array('style'=>"width: 100%;"))?>
        下图为邀请界面：
        <?=RHtml::showImage("files/images/site/07.png","",array('style'=>"width: 100%;"))?>
        ...
        <?=RHtml::showImage("files/images/site/08.png","",array('style'=>"width: 100%;"))?>
        你可以在该话题下与其他人讨论、交流以及点赞(绿色标记部分)：

        <?=RHtml::showImage("files/images/site/09.png","",array('style'=>"width: 100%;"))?>


        你也可以创建新的话题：
            <?=RHtml::showImage("files/images/site/10.png","",array('style'=>"width: 100%;"))?>
        如下图所示，赶快发表你的第一个话题吧~
        <?=RHtml::showImage("files/images/site/11.png","",array('style'=>"width: 100%;"))?>
        </div>
        ...
        本条目将指引你进行朋友管理，包括查找好友，新增好友，查看好友信息等操作。在个人主页下，点击“find friend”按钮（黄色标记部分），即可看到系统之中所有的用户，您可以通过直接输入用户名进行搜索。
            ...
            <?=RHtml::showImage("files/images/site/12.png","",array('style'=>"width: 100%;"))?>

        查找到您感兴趣的用户后，点击该用户头像，在该用户公开信息中，点击“add friend”即可申请好友：
            <?=RHtml::showImage("files/images/site/13.png","",array('style'=>"width: 100%;"))?>

        你也可以在这里查看该用户的公开信息。对于已成为好友的用户，您可以向对方发送message或解除好友关系：
            <?=RHtml::showImage("files/images/site/14.png","",array('style'=>"width: 100%;"))?>
        ...
        该条目将指导你对收到的信息进行管理，以及如何发送信息。
            ...
            <?=RHtml::showImage("files/images/site/15.png","",array('style'=>"width: 100%;"))?>
        如图所示，你可以在个人主页中对信息进行管理，在主页messages右侧，有一个标签，提示当前有多少条未读消息（黄色标记部分），你可以对收到的信息进行管理，如查看、回复、标记为已读、删除到垃圾箱中等（绿色标记部分）。你也可以对其他人发送消息（红色标记部分），界面如下图所示：
            <?=RHtml::showImage("files/images/site/16.png","",array('style'=>"width: 100%;"))?>
        你可以向任何已添加为好友的用户发送消息。
        </div>
        ...
        该条目将指引你对个人信息进行修改。如图，在个人主页下点击my profile即可查看个人信息，如下图所示：
            <?=RHtml::showImage("files/images/site/17.png","",array('style'=>"width: 100%;"))?>
        ...
        注意！只有VIP用户才有资格申请发布广告。关于如何成为VIP用户，详见profile指引。
            <?=RHtml::showImage("files/images/site/18.png","",array('style'=>"width: 100%;"))?>
        如上图所示，单击个人主页的“advertisement”条目，界面如上图所示。在这里，您可以发布诸如征友、销售、教育等广告信息，通过publish按钮提交，待管理员审核通过后，即可在屏幕右下方看到您的广告，如下图所示：
            <?=RHtml::showImage("files/images/site/19.png","",array('style'=>"width: 100%;"))?>
