...
<div class="panel panel-default">
    <div class="panel-body">
        ...
        <div class="user-profile-tiny">
            <?=$self->module('user_panel',array('userId'=>$user->id, 'viewUser' => true));?>
        ...
        if ($user->status == User::STATUS_BLOCKED) {
            ...
            <div class="panel-body">
                <ul class="nav nav-tabs">
                    <li <?php if ($part == 'joins') echo 'class="active"';?>><?=RHtml::linkAction('user','Joined Groups','view',$user->id)?></li>
                    <li <?php if ($part == 'posts') echo 'class="active"';?>><?=RHtml::linkAction('user','Posts','view',[$user->id, 'posts'])?></li>
                    <li <?php if ($part == 'likes') echo 'class="active"';?>><?=RHtml::linkAction('user','Favorites','view',[$user->id, 'likes'])?></li>
                    <li <?php if ($part == 'profile') echo 'class="active"';?>><?=RHtml::linkAction('user','Profile','view',[$user->id, 'profile'])?></li>
                </ul>
            </div>
            ...
            <div class="panel-body">
                ...
                <ul class="list-group">
                        ...
                        if (empty($userGroup)) {
                            ...
                            <div class="panel-my-group">
                                <div id="waterfall-groups" class="waterfall" style="margin: 0 -10px 0 -10px;">
                                    <?php
                                    $self->renderPartial("_common._groups_list",array('groups'=>$userGroup),false);
                                    ?>
                                </div>
                                <div class="clearfix"></div>
                                <div class="load-more-groups-processing" id="loading-groups">
                                    <div class="horizon-center">
                                        <img class="loading-24-24" src="<?=RHtml::siteUrl('/public/images/loading.gif')?>" /> loading...
                                    </div>
                                </div>
                            ...
                            <script>
                                ...
                                function loadMoreGroups(){
                                    ...
                                    //$('#load-more-groups').hide(0);
                                    $.ajax({
                                        url: "<?=RHtml::siteUrl('user/view/'.$user->id) ?>",
                                        type: "post",
                                        data:{page: ++curPage},
                    ...
                    <?php
                            ...
                            $self->renderPartial("_common._posts_table",array('posts'=>$postTopics,'showGroup'=>true),false);
                        } else if ($part == 'likes') {
                                $self->renderPartial("_common._posts_table",array('posts'=>$likeTopics,'showAuthor'=>true,'showGroup'=>true),false);
                            }
                    ?>
                    <div>
                        <?=isset($pager)?$pager:"" ?>
