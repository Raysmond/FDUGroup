...
<div class="panel panel-default">
    ...
    <div class="panel-body">
        <div class="navbar-left message-send">
            <a href="<?=RHtml::siteUrl('message/send')?>" title="send">
        ...
        <div class="navbar-right">
            <ul class="nav nav-tabs">
                <li <?=($type=='all'?'class="active"':"")?>><?=RHtml::linkAction('message','All messages','view','all')?></li>
                <li <?=($type=='unread'?'class="active"':"")?>><?=RHtml::linkAction('message','Unread messages','view','unread')?></li>
                <li <?=($type=='read'?'class="active"':"")?>><?=RHtml::linkAction('message','Read messages','view','read')?></li>
                <li <?=($type=='trash'?'class="active"':"")?>><?=RHtml::linkAction('message','Trash','view','trash')?></li>
            </ul>
                ...
                <div style="float:right;margin-top: -2px;">
                    ...
                    if($msg->receiverId==Rays::user()->id){
                        ...
                        if($msg->status!=Message::STATUS_TRASH){
                            $url = RHtml::siteUrl("message/trash/".$msg->id);
                            ?>
                            <a href="javascript:markStatus('<?=$url?>',<?=$msg->id?>)"><span class="glyphicon glyphicon-trash message-trash"></span></a>
                            <?php
                        }
                        if($type=='trash'){
//                            echo RHtml::linkAction('message',"",'delete',$msg->id,array('title' => 'Delete', 'class'=>'glyphicon glyphicon-remove message-trash'));
                            $url = RHtml::siteUrl("message/delete/".$msg->id);
                            ?>
                    ...
                    echo RHtml::linkAction('message',$title,'detail',$msg->id);
...
<script>
    $(document).ready(function () {
        $(".message-list .message-item .message-body a").click(function () {
            ...
            if (id) {
                $.ajax({
                    url: "<?=RHtml::siteUrl("message/read")?>",
                    type: "post",
                    data: {messageId: id},
