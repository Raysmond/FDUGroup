...
class GroupController extends BaseController
{
    ...
    public function actionBuild()
    {
        if (Rays::isPost()) {
            ...
            if ($validation->run()) {
                $group = Group::buildGroup($_POST['group-name'], $_POST['category'], RHtml::encode($_POST['intro']), $user->id);
    ...
    public function actionJoin($groupId) {  //by songrenchu: need censorship by group creator
        ...
        if ($group !== null) {
            ...
            $content = RHtml::linkAction('user',$userName,'view',$userId)." wants to join your group ".
                RHtml::linkAction('group', $group->name, 'detail', $group->id)
                ."<br/>"
                .RHtml::linkAction('group','Accept','accept', $censor->id,array('class'=>'btn btn-xs btn-success'))
                ."&nbsp;&nbsp;"
                .RHtml::linkAction('group','Decline','decline', $censor->id,array('class'=>'btn btn-xs btn-danger'));
