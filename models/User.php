...
class User extends RModel implements RAuthProvider
{
    ...
    public function sendWelcomeMessage()
    {
        ...
        $content = 'Dear ' . $this->name . " : <br/>"
            ...
            . '<br/>'
            . date('Y-m-d H:i:s');
        Message::sendMessage('system', 0, $this->id, $title, RHtml::encode($content), null, 1);
    }
    ...
    {
        if ($requiredAuthority == null || $requiredAuthority == Role::ANONYMOUS || $requiredAuthority == $this->authority())
            return true;
        if ($this->authority() == Role::VIP && $requiredAuthority == Role::AUTHENTICATED)
            return true;
        if ($this->authority() == Role::ADMINISTRATOR)
            return true;
        return false;
    }
    ...
    {
        return User::find("id", $identifier)->join("role")->first();
    }
}
